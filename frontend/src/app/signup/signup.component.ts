...
import { TeamTabsComponent } from '@app/admin/teams/team-tabs/team-tabs.component';
...
export class SignupComponent implements OnInit {
    ...
    doSave() {
        ...
        this.api.send('teams/profile', {id: this.profileId}, this.dataForm).then(res => {
            if (res['errors']) {
                ...
                if (this.profileId === 0) {
                    this.woFlash.addMessage('Team creation successful!');
                    this.router.navigate(['/signup', res['data'].id]);
                    this.dataErrors = {};
                } else {
                    this.dataErrors = {};
                    setTimeout(() => {
    ...
    doCancel() {
        this.router.navigate(['quiz/login']);
        //window.history.back();
